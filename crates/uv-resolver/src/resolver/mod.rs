...
impl<'a> PossibleForkGroups<'a> {
    ...
    fn remaining_universe(&self) -> Option<MarkerTree> {
        let have = self
            .forks
            .iter()
            .fold(MarkerTree::new_false(), |tree, fork| tree.or(fork.union()));
        let missing = have.negate();
        if !missing.is_satisfiable() {
            return None;
        }
        Some(missing)
    }
...
impl<'a> PossibleFork<'a> {
    ...
    fn union(&self) -> MarkerTree {
        self.packages
            .iter()
            .fold(MarkerTree::new_false(), |union, &(_, &tree)| union.or(tree))
    }
}
